...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      if (rememberMe) {
        localStorage.setItem("newsiq_remember_email", email);
      } else {
        localStorage.removeItem("newsiq_remember_email");
      }
      router.push(redirect);
    }
  };
  ...
  useEffect(() => {
    const remembered = localStorage.getItem("newsiq_remember_email");
    if (remembered) {
      setEmail(remembered);
      setRememberMe(true);
    }
      ...
      >
        ...
        <div className="flex flex-col items-center mb-6 text-center">
          ...
          <h2 className="text-2xl font-black tracking-tight text-white">
            Log in to News<span className="text-red-500">IQ</span>
          </h2>
          <p className="text-slate-400 text-xs mt-1">
            Access customized alerts, bookmarks, and finance streams.
          </p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={cn(
                  "w-full bg-[#060c18] border text-slate-200 placeholder:text-slate-500 rounded-xl pl-10 pr-10 py-2.5 outline-none text-xs transition-all",
        ...
        <div className="flex items-center gap-3 my-5">
          ...
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">or sign in with</span>
          <div className="h-px bg-white/[0.08] flex-1" />
        </div>
